        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles mobile-container-padding heading_style">
                    ...
                    if ($filter_table !== '') {
                        $sql .= " AND (a.title LIKE '%" . $conn->real_escape_string($filter_table) . "%' OR a.title_it LIKE '%" . $conn->real_escape_string($filter_table) . "%' OR a.title_de LIKE '%" . $conn->real_escape_string($filter_table) . "%')";
                    }

                    // Count total records with filter
                    $sqlCount = "SELECT COUNT(*) AS total FROM `tbl_handbook` as a WHERE a.`hotel_id` = $hotel_id and a.is_delete = 0 and a.saved_status = 'DRAFT'";

                    if ($filter_table !== '') {
                        $sqlCount .= " AND (a.title LIKE '%" . $conn->real_escape_string($filter_table) . "%' OR a.title_it LIKE '%" . $conn->real_escape_string($filter_table) . "%' OR a.title_de LIKE '%" . $conn->real_escape_string($filter_table) . "%')";
                    }
                    ...
                    // Final SQL query with pagnation
                    $sql .= " ORDER BY a.hdb_id DESC LIMIT $offset, $recordsPerPage";
                                                ...
                                                while ($row = mysqli_fetch_array($result10)) {
                                                    ...
                                                    <tr id="<?php echo $i; ?>">
                                                        ...
                                                        <td class="font-size-subheading text-center"><a
                                                                href="edit_handbook.php?id=<?php echo $row['hdb_id']; ?>"><i
